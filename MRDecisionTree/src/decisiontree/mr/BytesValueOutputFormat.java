...
public class BytesValueOutputFormat extends
		FileOutputFormat<NullWritable, BytesWritable> {
	...
	public RecordWriter<NullWritable, BytesWritable> getRecordWriter(
		...
		if (isCompressed) {
			Class<? extends CompressionCodec> codecClass = getOutputCompressorClass(
					taskAttemptContext, GzipCodec.class);
		...
		if (!isCompressed) {
			...
			FSDataOutputStream fileOut = fs.create(file, false);
			return new ByteRecordWriter(new DataOutputStream(
					codec.createOutputStream(fileOut)));
		}
	}

	protected static class ByteRecordWriter extends
